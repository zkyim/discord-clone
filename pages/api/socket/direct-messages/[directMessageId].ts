...
export default async function handler(req: NextApiRequest, res: NextApiResponseServerIo) {
  ...
  try {
    ...
    if (!directMessageId) return res.status(400).json({ error: "directMessage Id missing" });
    ...
    if (!directMessage || directMessage.deleted) return res.status(400).json({ error: "Message not found" });
    ...
    console.log("[MESSAGE_ID_PATCH]", err);
